...
import {loadActivityBar} from "../../activities/activity.js";

export function initCommentButtons(dataList) {
    const user = getUser();
    ...
    commentContainers.forEach(commentContainer => {
        ...
        loadActivityBar(activityBar);
        
        const replyButton = activityBar.querySelector(':scope > [data-component="comment-reply"]');
        if (replyButton) replyButton.addEventListener('click', () => replyComment(commentContainer, id));

        const viewReplyButton = activityBar.querySelector(':scope > [data-component="view-reply"]');
        if (viewReplyButton) viewReplyButton.addEventListener('click',async () =>await showReply(commentContainer));
