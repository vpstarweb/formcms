...
) : IBookmarkService
{
    ...
    public async Task UpdateFolder(long id,BookmarkFolder folder, CancellationToken ct)
    {
        ...
        var affected = await ctx.EngagementStatusShardRouter.PrimaryDao(userId).Exec(folder.Update(),ct);
    ...
    public async Task DeleteFolder(long folderId, CancellationToken ct)
    {
        ...
        var dao = ctx.EngagementStatusShardRouter.PrimaryDao(userId);
        var executor = ctx.EngagementStatusShardRouter.PrimaryDao(userId);
    ...
    public Task DeleteBookmark(long bookmarkId, CancellationToken ct)
    {
        ...
        return ctx.EngagementStatusShardRouter.PrimaryDao(userId).Exec(query, ct);
    }
    ...
    {
        var records = await ctx.EngagementStatusShardRouter.ReplicaDao(userId).Many(BookmarkFolders.All(userId), ct);
        ...
        CancellationToken ct)
    {
        ...
        var existing = await ctx.EngagementStatusShardRouter.ReplicaDao(userId).Many(getExistingQuery, ct);
