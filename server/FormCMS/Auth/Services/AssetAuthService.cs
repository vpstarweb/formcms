    ...
    ):IAssetAuthService
{
    public Asset PreAdd(Asset asset)
    {
        ...
        return asset with{CreatedBy = profileService.GetInfo()!.Id};
    }
    ...
    public ImmutableArray<Filter> PreList(ImmutableArray<Filter> filters)
    {
        ...
        var constraint = new Constraint(Matches.EqualsTo, [profileService.GetInfo()!.Id]);
    ...
    private async Task EnsureCreatedByCurrentUser(long recordId)
    {
        ...
        if (record is null || record.StrOrEmpty(nameof(Asset.CreatedBy).Camelize()) != profileService.GetInfo()!.Id)
        {
