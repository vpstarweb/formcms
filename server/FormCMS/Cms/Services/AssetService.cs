...
) : IAssetService
{
    ...
    public async Task<string> ConvertToMp3(long id, CancellationToken ct)
    {
        ...
        var msg = JsonSerializer.Serialize(new ConvertVideoMessage(asset.Name, asset.Path, "mp3", false, newPath, userId));
    ...
    public async Task CreateNewAssetRefOriginal(string oldPath, string newPath, CancellationToken ct)
    {
        ...
        var newFileMetadata = await store.GetMetadata(newPath, ct)
                                ?? throw new ResultException($"Failed to get metadata for duplicated file: {newPath}");

        // Create a new Asset record
        var newAsset = existingAsset with
        {
            ...
            Type = newFileMetadata.ContentType,
            CreatedBy = userId,
            CreatedAt = DateTime.UtcNow,
            UpdatedAt = DateTime.UtcNow,
            Progress = 0 // Reset progress for new asset
        };
    ...
    public async Task Duplicate(string oldPath, string newPath, CancellationToken ct)
    {
        var existingAsset = await Single(oldPath, false, ct);
        var userId = identityService.GetUserAccess()?.Id ?? throw new ResultException("Access denied");

        // Duplicate the file in the file store
        await store.Duplicate(oldPath, newPath, ct);
